...
def PatchedCalculateFunc(func: Callable, long_leg=[], short_leg=[], return_all=False, *args, **kwargs):
    ...
    long_leg_thread = Thread(
        target=get_func_values, args=(long_leg, "long", *args), kwargs=kwargs, name=f"{func.__name__}_long"
    )
    short_leg_thread = Thread(
        target=get_func_values, args=(short_leg, "short", *args), kwargs=kwargs, name=f"{func.__name__}_long"
    )
    ...
    if func.__name__ == "pct_spot_slides":
        ...
        if return_both_data:
            structure_dict["total"] = sum([x[1] for x in structure_dict["long"]]) + sum(
                [x[1] for x in structure_dict["short"]]
            )
        else:
            structure_dict["total"] = sum(structure_dict["long"]) + sum(structure_dict["short"])
...
class Calculate:
    ...
    def init_rfrate_ts(cls):
        cls.rf_ts = get_risk_free_rate_helper_v2()
    ...
    def pv(
        ...
        if asset is None:
            ...
            if asset.__class__.__name__ == "Stock":
                raise Exception(
                    "Stock can't be priced with options model. This method isn't utilized for a Stock Instance"
                )
    ...
    def pct_spot_slides(
        asset=None,
        pct_spot=[0.8, 0.9, 0.95, 1, 1.05, 1.1, 1.2],
        greeks_to_calc=["delta", "gamma"],
        asset_type=None,
        **kwargs,
    ):
        """
        ...
        **kwargs = Keyword arguments as seen in Calculate.PV()
        ...
        if asset is None:
            ...
            assert asset_type.lower() in ["stock", "option"], (
                f'Invalid asset_type, expected "stock" or "option", recieved "{asset_type}"'
            )
            if asset_type.lower() == "stock":
                ...
                if kwargs:
                    ...
                    start = kwargs["start"]
                    modelType = "option"
                    return Calculate.scenario_helper(
                        "spot",
                        pct_spot,
                        K=k,
                        S0=s0,
            ...
            return PatchedCalculateFunc(
                Calculate.pct_spot_slides,
                long_leg=asset.long,
    ...
    def pct_vol_slides(
        asset=None,
        pct_spot=[-0.05, -0.02, -0.01, 0, +0.01, +0.02, +0.05],
        greeks_to_calc=["delta", "gamma"],
        asset_type=None,
        **kwargs,
    ):
    ...
    def spot_vol_grid(
        ...
        **kwargs = Keyword arguments as seen in Calculate.PV()
        ...
        if asset is None:
            ...
            assert asset.__class__.__name__ == "Option", (
                f"Invalid asset type, expected Option, recieved {asset.__class__.__name__}"
            )
            k = getattr(asset, OptionModelAttributes.K.value)
            exp_date = getattr(asset, OptionModelAttributes.exp_date.value)
            sigma = getattr(asset, OptionModelAttributes.sigma.value)
        ...
        def spot_vol_helper(spot_shock, vol_shock, greek_enum):
            if greek_enum not in ["pv", "delta", "gamma", "vega", "rho", "theta", "vanna", "volga", "pnl"]:
                raise TypeError(
                    f'Invalid greek_enum, expected one of ["pv", "delta", "gamma", "vega", "rho", "theta", "vanna", "volga"], recieved {greek_enum}'
                )
            ...
            if greek_enum in ["pv", "pnl"]:
                shocked_value = fn(
                    K=k, exp_date=exp_date, sigma=shocked_vol, S0=shocked_spot, put_call=put_call, r=r, y=y, start=start
                )
            else:
                shocked_value = fn(
                    K=k, exp=exp_date, sigma=shocked_vol, S=shocked_spot, flag=put_call, r=r, start=start, y=y
                )
            ...
            if greek_enum == "pnl":
                ...
                pv = fn(K=k, exp_date=exp_date, sigma=sigma, S0=s0, put_call=put_call, r=r, y=y, start=start)
    ...
    def scenario_helper(type_, pct_spot, **kwargs):
        ...
        if type_ == "spot":
            ...
            if modelType == "option":
                ...
                scen["shocked_pv"] = scen.apply(
                    lambda x: Calculate.pv(
                        K=k,
                        exp_date=exp_date,
                        sigma=sigma,
                        S0=x["shocks"] * s0,
                ...
                scen["pv"] = scen.apply(
                    lambda x: Calculate.pv(
                        K=k, exp_date=exp_date, sigma=sigma, S0=s0, put_call=put_call, r=r, y=y, start=start
                    ),
                    axis=1,
                )
                ...
                if greeks_to_calc:
                    for greek in greeks_to_calc:
                        ...
                        scen[greek] = scen.apply(
                            lambda x: fn(
                                K=k, exp=exp_date, sigma=sigma, S=s0 * x["shocks"], flag=put_call, r=r, start=start, y=y
                            ),
                            axis=1,
                        )
            ...
            if modelType == "option":
                ...
                if greeks_to_calc:
                    for greek in greeks_to_calc:
                        ...
                        scen[greek] = scen.apply(
                            lambda x: fn(
                                K=k, exp=exp_date, sigma=sigma + x["shocks"], S=s0, flag=put_call, r=r, start=start, y=y
                            ),
                            axis=1,
                        )
    ...
    def delta(asset=None, S=None, K=None, r=None, sigma=None, start=None, flag=None, exp=None, y=None, model="bs"):
        ...
        if isinstance(asset, Option) or asset.__class__.__name__ == "Option":
            ...
            args_str = [
                OptionModelAttributes.S0.value,
            ...
            assert all(v is not None for v in [S, K, r, sigma, start, flag, exp, y]), (
                f"None of y, S, K, r, sigma, start, flag, exp, can be None"
            )
            if sigma == 0:
    ...
    def greeks(asset=None, S=None, K=None, r=None, sigma=None, start=None, flag=None, exp=None, y=None, model="bs"):
        ...
        if isinstance(asset, Option) or asset.__class__.__name__ == "Option":
            try:
                greeks = {
                    "Delta": Calculate.delta(asset, S=S, K=K, r=r, sigma=sigma, start=start, model=model),
                    ...
                    "Theta": Calculate.theta(asset, S=S, K=K, r=r, sigma=sigma, start=start, model=model)
                    if Calculate.theta(asset, S=S, K=K, r=r, sigma=sigma, start=start, model=model) is not None
                    else 0,
            ...
            try:
                greeks = {
                    "Delta": Calculate.delta(
                        S=S, K=K, r=r, sigma=sigma, start=start, flag=flag, exp=exp, y=y, model=model
                    ),
                    ...
                    if Calculate.theta(S=S, K=K, r=r, sigma=sigma, start=start, flag=flag, exp=exp, y=y, model=model)
                    is not None
                    else 0,
            ...
            raise Exception(f"Greeks cannot be Calculated for {asset} type")
    ...
    def attribution(
        ...
        **kwargs,
        ...
        if asset.__class__.__name__ == "Option":
            ...
            ## GET OPTION TIMESERIES
            spot_ts = asset.spot(
                ts_start=start, ts_end=end, ts_timeframe=ts_timeframe, ts_timewidth=ts_timewidth, ts=True
            )[spot_col]
            ...
            if replace == "partial":
                ...
                raise Exception(
                    f"Invalid replace option. Expected 'partial', 'close', 'default_fill', recieved {replace}"
                )

            ## Fill missing data + Rename columns to Option_Close
            full_data.rename(columns={"Close": "Option_Close"}, inplace=True)
            full_data.loc[close_fill_mask, "Option_Close"] = full_data.loc[
                close_fill_mask, asset.default_fill.capitalize()
            ]

            ## Get Vol Timeseries
            vol = asset.vol(ts_start=start, ts_end=end, ts_timeframe=ts_timeframe, ts_timewidth=ts_timewidth, ts=True)[
                vol_col
            ]
            ...
            full_data["RF_rate"] = get_risk_free_rate_helper_v2(intv)["annualized"]
            ...
            full_data[[f"{x}_Change_Percent" for x in spot_col_2 + ["Stock_Close"]]] = full_data[
                spot_col_2 + ["Stock_Close"]
            ].pct_change()
            ...
            if method == "GB":
                ...
                full_data["total_seconds"] = (
                    full_data["Datetime"] - full_data["Datetime"].shift(1)
                ...
                PnL_Data["Vanna_PnL"] = (
                    (full_data["Vanna"] * 100) * full_data["Stock_Close_Change_Mark"] * full_data["Vol_Change_Mark"]
                )
                ...
                PnL_Data = full_data.apply(
                    lambda x: fullRevalPnL(
                        {
                            "S0": x["prev_day_Stock_Close"],
                            "K": asset.K,
                            "rf_rate": x["prev_day_RF_rate"],
                            "sigma": x[f"prev_day_{vol_col[0]}"],
                            "start": x["prev_day_Datetime"],
                            "put_call": asset.put_call,
                            "exp": asset.exp,
                            "y": asset.y,
                            "price": x["prev_day_Option_Close"],
                        },
                        {
                            "S0": x["Stock_Close"],
                            "K": asset.K,
            ...
            return PatchedCalculateFunc(
                ...
                method=method,
                replace=replace,
                return_both_data=return_both_data,
            )
...
def fullRevalPnL(
    ...
    assert start_dict.keys() == end_dict.keys(), (
        f"Keys in both dictionaries must be the same. Expected {start_dict.keys()} recieved {end_dict.keys()}"
    )
    assert start_dict["K"] == end_dict["K"], (
        f"Strike Price must be the same. Expected {start_dict['K']} recieved {end_dict['K']}"
    )

    S0, S1, price0, price1 = start_dict["S0"], end_dict["S0"], start_dict["price"], end_dict["price"]
    K, r0, r1, sigma0, sigma1, y0, y1 = (
        start_dict["K"],
        start_dict["rf_rate"],
        end_dict["rf_rate"],
        start_dict["sigma"],
    ...
    sigma0_pv_bump = Calculate.pv(
        S0=S0_bump, K=K, r=r0, sigma=sigma_bump, start=start, put_call=put_call, exp_date=exp, y=y0
    )
    sigma_change_pv = Calculate.pv(S0=S0, K=K, r=r0, sigma=sigma1, start=start, put_call=put_call, exp_date=exp, y=y0)
    sigma_plus_spot_pv = Calculate.pv(
        S0=S1, K=K, r=r0, sigma=sigma1, start=start, put_call=put_call, exp_date=exp, y=y0
    )
