...
class Stock:
    ...
    def spot(
        ...
        if spot_type == "chain_price":
            df = retrieve_timeseries(
                self.ticker,
                end=change_to_last_busday(datetime.today()).strftime("%Y-%m-%d"),
                start="1960-01-01",
                interval=interval,
                provider=provider,
                spot_type=spot_type,
    ...
    def option_chain(self, date=None):
        ...
        contracts_v2 = contracts.pivot_table(
            index=["Expiration", "DTE", "Strike"], columns="Right", values="Root", aggfunc="count"
        )
    ...
    def get_next_earnings(self, robin_stocks_credentials=None):
        try:
            if robin_stocks_credentials is not None:
                login = robin.login(robin_stocks_credentials["name"], robin_stocks_credentials["password"])
            ...
            if earnings is None:
                raise ValueError("robin stocks returned no earnings, resolving to alpha vantage")
            for earning in earnings:
                ...
                if earning["year"] == current_year and earning["quarter"] == quarter["period"]:
                    if int(earning["report"]["date"].split("-")[1]) < int(
                        quarter["date"]["start"].split("-")[0]
                    ) or int(earning["report"]["date"].split("-")[1]) > int(quarter["date"]["end"].split("-")[0]):
                        raise ValueError(
            ...
            if earning_report:
                ...
                days_to_earning = datetime(int(date_split[0]), int(date_split[1]), int(date_split[2])) - datetime(
            ...
            try:
                ...
                difference = datetime.strptime(end, "%Y-%m-%d") - datetime.strptime(start, "%Y-%m-%d")
    ...
    def get_earnings_ts(self):
        try:
            ...
            earning_dates_tds = soup.find_all("td", {"aria-label": "Earnings Date"})
            date_strings = [" ".join(span.stripped_strings) for span in earning_dates_tds]
            date_strings = [date_str.replace("EDT", "EST") for date_str in date_strings]
            dates = pd.to_datetime(date_strings, format="%b %d, %Y, %I %p %Z", errors="coerce")
            ...
            df = pd.DataFrame(
                {
                    "Date": dates,
                    "After Hours": dates > market_close,
                    "Before Hours": dates < market_open,
                    "During": (dates >= market_open) & (dates <= market_close),
                }
            )
    ...
    def get_typical_price(self, start, end):
        ...
        data["typical_money_flow"] = (data["High"] + data["Low"] + data["Close"]) / 3
        return data
    ...
    def get_money_flow_ratio(self, start, end, window=2):
        ...
        money_flow_negative = money_flow_s.rolling(window).apply(lambda x: np.sum(np.where(x < 0.0, x, 0.0)), raw=True)
    ...
    def get_money_flow_index(self, start, end):
        ...
        data["money_flow_index"] = 100.0 - (100.0 / (1 + data["money_flow_ratio"]))
        return data
