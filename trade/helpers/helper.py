        ...
        try:
            ...
            if "split_ratio" not in data.columns:
                ...
                if spot_type == "close":
                    data["split_ratio"] = 1
                    logger.info(
                        f"No splits found for {tick} between {start} and {end}. Added split_ratio column with 1s"
                    )
                ...
                ## `chain_price` spot type means split_ratio is important. Retry up to 3 times
                    ...
                    if "split_ratio" not in data.columns:
                        raise YFinanceEmptyData(
                            f"yfinance returned data without split_ratio column for {tick} after 3 retries"
                        )
...
def print_cprofile_internal_time_share(_stats, top_n=20, sort_by="tottime", full_name=False):
    ...
    print(f"{'Function':<100} {'SelfTime':>10} {'ShareOfTotal':>12}")
    print("-" * 115)
    ...
    for func, stat in top_list:
        ...
        print(f"{label:<100} {self_time:>10.4f} {ratio:>12.2%}")
...
def print_top_cprofile_stats(_stats, top_n=20, sort_by="cumulative", full_name=False):
    ...
    # Header
    print(f"{'Function':<125} {'CumTime':>10} {'RatioToTop':>12}")
    print("-" * 150)
    ...
    for func, stat in top_list:
        ...
        print(f"{label:<125} {cum_time:>10.4f} {ratio:>12.2f}")


def find_split_dates_within_range(tick: str, start: str, end: str):
    """
...
def implied_vol_bt(
    ...
    if pd.isna(implied_vol) or implied_vol == 0.0:
        logger.warning(
            f"Binomial Implied vol is NaN for {S0}, {K}, {r}, {market_price}, Exp: {exp_date},  Flag: {flag}, Start: {start}"
        )
        return 0.0
    return implied_vol
...
def optionPV_helper(
    ...
    try:
        ...
        maturity_date = ql.Date(
            pd.to_datetime(maturity_date_str).day,
        ...
        settlement_date = ql.Date(
            pd.to_datetime(settlement_date_str).day,
...
def to_datetime(
    ...
    If input is already a datetime object, return as is.
    For iterables, uses pd.to_datetime.

    Args:
        date_input: String, datetime, or iterable to convert.
        format: Optional strftime format. If None, tries "%Y-%m-%d" first, then lets pandas guess.

    Returns:
        datetime object for single input, DatetimeIndex for iterables.
