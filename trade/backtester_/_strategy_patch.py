...
def collect_signals(strategy_class):
    """
    ...
    class SignalCollectingStrategy(strategy_class):
        ...
        def _register_signals(self):
            ...
            if hasattr(self, "is_open_signal"):
                ...
                if open_signal:
                    signal = {"type": "open", "date": dt, "price": self.data.Close[-1], "info": "Open signal generated"}
                    self.signal_collector.register_signal(signal)
            else:
                raise NotImplementedError(
                    "The strategy must implement 'should_open' method. "
            ...
            if hasattr(self, "is_close_signal"):
                if not callable(getattr(self, "should_close")):
                    raise NotImplementedError("The 'should_close' attribute must be callable.")
                ...
                raise NotImplementedError(
                    "The strategy must implement 'should_close' method. "
                    "This is required for signal collection. "
                    "It should be a method that returns a boolean indicating "
            ...
            if not open_signal and not close_signal:
                ...
                signal = {"type": "none", "date": dt, "price": self.data.Close[-1], "info": "No signal generated"}
                self.signal_collector.register_signal(signal)
