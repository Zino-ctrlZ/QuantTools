...
def chain_cache_key(root: str, valuation_date: str | datetime, div_type: DivType, vol_type: VolType) -> str:
    """
...
def assert_k_bounds_model_range(k: list | np.ndarray, f: float | np.ndarray) -> None:
    """
    ...
    if not within:
        raise ValueError((f"Strikes {k} are out of bounds. " f"Must be between {min_m} and {max_m}."))


def handle_strikes(
    k: list | np.ndarray, f: list | float, strike_type: Literal["p", "k", "pf", "f"], spot: float = None
) -> np.ndarray:
    """
...
def calculate_normalized_rmse_loss(
    ...
    right_wing_loss = (
        np.sqrt(np.mean((market_iv[right_wing_mask] - model_iv[right_wing_mask]) ** 2)) / median_right_wing_iv
    )

    ## Left Wing Loss (< 1.0)
    left_wing_mask = moneyness < 1.0
    median_left_wing_iv = np.median(market_iv[left_wing_mask])
    left_wing_loss = np.sqrt(np.mean((market_iv[left_wing_mask] - model_iv[left_wing_mask]) ** 2)) / median_left_wing_iv

    return normalized_median_loss, right_wing_loss, left_wing_loss


def calculate_normalized_mae_loss(
    market_iv: np.ndarray, model_iv: np.ndarray, moneyness: np.ndarray
) -> Tuple[float, float, float]:
    """
    ...
    right_wing_mask = moneyness > 1.0
    median_right_wing_iv = np.median(market_iv[right_wing_mask])
    right_wing_mae_loss = np.mean(np.abs(market_iv[right_wing_mask] - model_iv[right_wing_mask])) / median_right_wing_iv

    ## Left Wing Loss (< 1.0)
    left_wing_mask = moneyness < 1.0
    median_left_wing_iv = np.median(market_iv[left_wing_mask])
    left_wing_mae_loss = np.mean(np.abs(market_iv[left_wing_mask] - model_iv[left_wing_mask])) / median_left_wing_iv
...
def identify_length_for_model(string, integer) -> int:
    ...
    assert string in TIMEFRAMES_VALUES.keys(), (
        f"Available timeframes are {list(TIMEFRAMES_VALUES.keys())}, " f'received "{string}"'
    )
    return integer * TIMEFRAMES_VALUES[string]


def convert_date_to_time_to_maturity(dt: str, valuation_date: str = None) -> float:
    """
    ...
    if isinstance(dt, (str, pd.Timestamp, datetime, date)):
        ...
        raise ValueError(
            "Unsupported type for dt: " f"{type(dt)}. Expected str, int, float, datetime, or pd.Timedelta."
        )
...
def assert_dt_within_range(dt: float):
    ...
    if not (min_dte <= dt <= max_dte):
        raise ValueError((f"Time to maturity {dt} is out of bounds. " f"Must be between {min_dte} and {max_dte}."))
