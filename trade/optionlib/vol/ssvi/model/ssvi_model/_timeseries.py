...
from trade.helpers.helper import is_weekend, not_trading_day, check_missing_dates, assert_member_of_enum
from trade.optionlib.vol.ssvi.model.ssvi_model._ssvi import _SSVIModel
from trade.optionlib.vol.ssvi.model.ssvi_model._eod_ssvi import EODMarketSSVIModel
from trade.optionlib.vol.ssvi.model.chain import MarketChainLoader
from trade.optionlib.config.types import VolType, DivType, VolSide
...
class SsviTimeseriesEOD(BaseModel, SingletonMixin):
    ...
    models: Optional[dict[str, _SSVIModel]] = Field(
        default=None, description="Dictionary of SSVI models for different option sides"
    )
    ...
    def _prepare_inputs(
        ...
        right: VolSide,
    ) -> tuple[np.ndarray, np.ndarray, VolSide]:
        """
    ...
    def _ensure_missing_models_loaded(
        self, all_dates: pd.DatetimeIndex, start_ts: pd.Timestamp, end_ts: pd.Timestamp
    ) -> None:
        """
    ...
    def predict(
        ...
        logger.info(
            "Prepared date range from %s to %s with %d business days",
