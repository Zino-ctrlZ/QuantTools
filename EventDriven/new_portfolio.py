...
class OptionSignalPortfolio(Portfolio):
    ...
    def max_contract_price(self, max_contract_price):
        ...
        for s in max_contract_price.keys():
            if max_contract_price[s] > self.allocated_cash_map[s]:
                raise ValueError(
                    f"max_contract_price for {s} cannot be greater than allocated cash of {self.allocated_cash_map[s]}"
                )
        ...
        if weight_map is not None:
            ...
            if len(unprocessed_symbols) > 0:
                self.logger.critical(
                    f"The following symbols: {unprocessed_symbols} are not being processed but present in weight_map"
                )
    ...
    def __construct_max_contract_price(self):
        ...
        if self.cash_allocator_config is not None:
            try:
                ...
                self.logger.warning(f"Falling back to default max_contract_price due to allocator error: {exc}")
        return {
            s: self.__normalize_dollar_amount_to_decimal(self.allocated_cash_map[s] * 0.5) for s in self.symbol_list
    ...
    def generate_order(self, signal_event: SignalEvent):
        ...
        if signal_type != "CLOSE":  # generate order for LONG or SHORT
            ...
            if signal_event.signal_id not in self.current_positions[symbol]:
                self.logger.warning(
                    f"No contracts held for {symbol} to sell at {signal_event.datetime}, Inputs {locals()}"
                )
            ...
            if skip:
                ...
                if isinstance(signal_event.parent_event, RollEvent):
                    self.logger.warning(
                        f"Not generating order because: CLOSE price is negative {signal_event}, skipping sell for roll event"
                    )
                    return None
                ...
                new_signal = SignalEvent(
                    signal_event.symbol,
                    next_trading_day,
                    SignalTypes.CLOSE.value,
                    signal_id=signal_event.signal_id,
                    parent_event=signal_event.parent_event,
                )
                self.logger.warning(
                    f"Not generating order because: CLOSE price is negative {signal_event}, moving event to {next_trading_day}"
                )
    ...
    def create_order(self, signal_event: SignalEvent, position_type: str, order_type: str = "MKT"):
        ...
        if self._is_holiday(signal_event.datetime):
            self.logger.critical(
                f"Market is closed on {signal_event.datetime}, cannot create order for signal: {signal_event}"
            )
        ...
        if self.logger.isEnabledFor(logging.DEBUG):
            self.logger.info(
                f"Cash at Hand: {cash_at_hand}, Max Contract Price: {max_contract_price} for Signal: {signal_event.signal_id}"
            )
        ...
        if position_state is None:
            position_state = self.risk_manager.get_order(
                OrderRequest(
                    date=date_str,
                    symbol=signal_event.symbol,
            ...
            self.position_cache[signal_event.signal_id] = (
                position_state  # backward compatibility with previous cache usage
            )
    ...
    def analyze_signal(self, event: SignalEvent):
        ...
        if not self.allow_multiple_trades and event.signal_type != SignalTypes.CLOSE.value:
            ...
            if has_open_position:
                self.logger.warning(
                    f"Pushing signal {event} to next trading day because a position already exists for {event.symbol}"
                )
    ...
    def analyze_multiasset_strategy(self, dt: Optional[pd.Timestamp] = None):
        ...
        exec_date = change_to_last_busday(
            dt + pd.tseries.offsets.BDay(self.t_plus_n), offset=-1, time_of_day_aware=False
        )
        ...
        for ticker, signal in opens.items():
            if signal.signal_id is None:
                raise ValueError(
                    f"Signal {signal} does not have signal_id, cannot process signal for multi-asset strategy"
                )
    ...
    def execute_roll(self, roll_event: RollEvent):
        ...
        sell_signal_event = SignalEvent(
            roll_event.symbol,
            roll_event.datetime,
            SignalTypes.CLOSE.value,
            signal_id=roll_event.signal_id,
            parent_event=roll_event,
        )
    ...
    def execute_roll_buy(self, roll_event: RollEvent):
        ...
        buy_signal_event = SignalEvent(
            roll_event.symbol, roll_event.datetime, roll_event.signal_type, signal_id=roll_event.signal_id
        )
    ...
    def update_positions_on_fill(self, fill_event: FillEvent):
        ...
        if fill_event.direction == "BUY":
            if fill_event.position is not None:
                ...
                if (
                    self.current_positions[fill_event.symbol] is not None
                    and fill_event.signal_id in self.current_positions[fill_event.symbol]
                ):
                    new_position_data["quantity"] = (
                        self.current_positions[fill_event.symbol][fill_event.signal_id]["quantity"]
                        + fill_event.quantity
                    )
                else:
                ...
                if self.eq_strategy is not None:
                    entry_price = self.__normalize_dollar_amount(fill_event.fill_cost)
        ...
        if fill_event.direction == "SELL":
            if fill_event.position is not None:
                ...
                if (
                    ...
                    return ValueError(
                        f"No position found for {fill_event.symbol} with signal_id {fill_event.signal_id}"
                    )
                new_position_data["market_value"] = self.__normalize_dollar_amount(fill_event.market_value)
        ...
        for sym in self.symbol_list:
            ...
            for signal_id in self.current_positions[sym]:
                ...
                if "exit_price" not in self.current_positions[sym][signal_id]:
                    new_weighted_holdings_entry[sym] += (
                        market_value  # update the holdings value to the market value of position + left over allocated cash
                    )

                # update positions
                if (
                    "exit_price" in self.current_positions[sym][signal_id]
                ):  # if position is closed, remove the signal from current_positions
                    # remove signal
    ...
    def get_all_positions(self) -> pd.DataFrame:
        ...
        for position_dict in all_positions_copy:
            ...
            for (
                symbol,
                positions,
            ) in position_dict.items():  # TODO:all positions structure now by signal, get symbol from position
