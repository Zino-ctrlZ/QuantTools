...
# from EventDriven.portfolio import Portfolio
...
## 3. I'm not a big fan of indexing with Timestamp. Pay attention to this in the future.
...
def populate_cache(order_candidates: dict, 
                   start_date: str = '2024-03-12',
                   end_date: str = '2024-03-14',
                   target_date: str = '2023-03-13') -> str|None:
    ...
    if is_holiday(target_date):
        ...
        # eod_results = retrieve_bulk_eod(symbol = symbol, exp = expiration, start_date = start_date, end_date = end_date)
        # eod_results['opttick'] = eod_results.apply(lambda x: generate_option_tick_new(symbol = x['Root'], exp = x['Expiration'].strftime('%Y-%m-%d'), strike = x['Strike'], right = x['Right']), axis = 1)
        # oi_results = retrieve_bulk_open_interest(symbol = symbol, exp = expiration, start_date = start_date, end_date = end_date)
        # oi_results['opttick'] = oi_results.apply(lambda x: generate_option_tick_new(symbol = x['Root'], exp = x['Expiration'].strftime('%Y-%m-%d'), strike = x['Strike'], right = x['Right']), axis = 1)
        ...
        # for key in eod_results.opttick.unique():
        #     close_cache[key] = eod_results[eod_results.opttick == key]
        #     oi_cache[key] = oi_results[oi_results.opttick == key]
