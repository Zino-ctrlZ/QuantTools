...
#     print(f"In `build_spread_by_ticks`: Rights filtered for: {schema['option_type'].lower()}. Found: {df['right'].unique()} types.")
#     try:
#         df["mid"] = df["chain_id"].map(cache)
#     except:
#         logger.error("Error mapping chain_id to mid prices in cache.")
#         df["mid"] = df["midpoint"]
...
#             except: ## Error handling for missing bid/ask data
#                 spread_bid = 0.0
#                 spread_ask = 0.0
...
#         pick = min((s for s in spreads if s["spread_price"] > 0), key=lambda s: (s["spread_pct_ratio"], -s['spread_oi'], s["spread_price"]), default=None)
#     else:
#         pick = min((s for s in spreads if s["spread_price"] < 0), key=lambda s: (s["spread_pct_ratio"], -s['spread_oi'], s["spread_price"]), default=None)
