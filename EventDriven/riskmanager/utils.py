...
def register_info_stack(id, data, data_col, update_kwargs = None):
    ...
    for k in data_col:
        ...
        info[f'{k.upper()}_MAX_STREAK'] = copy_cat[copy_cat[f"{k}_skip_day"] ==True].streak.max() if not copy_cat[copy_cat[f"{k}_skip_day"] ==True].streak.empty else 0 # noqa
...
# UNUSED: ThetaData wrappers moved to bottom archive
...
# UNUSED: helper not referenced in .py modules


# UNUSED: patch ticker toggles kept for reference
...
# # UNUSED: getter retained for reference
...
# UNUSED spike flag variants retained for reference
# '''
...
## Unused cache/chain/order helpers have been archived to the bottom of this file.


# UNUSED runtime cache/chain helpers
# # UNUSED: deprecated cache population helpers retained for reference
...
# # UNUSED: order candidate generator retained for reference
...
# # UNUSED liquidity check archived below

# ## ---------------------------------------------------------------------------
# ## UNUSED ARCHIVE (kept for reference only; not executed)
# if False:
...
#     def assemble_bulk_data_request(self, start: str | datetime, end: str | datetime, interval: str = "1d",
#                                    type_: str = "spot", strikes_right: List[Tuple] = None, model: str = "bs",
#                                    extra_cols: list = None) -> BulkOptionQueryRequestParameter:
#         strikes_right = strikes_right or []
...
#         data_request = BulkOptionQueryRequestParameter(
#             table_name=table,
#             db_name=database,
#             start_date=start,
#             end_date=end,
...
#         refresh_cache()
#         global oi_cache, close_cache, spot_cache
#         key = f"{x.Optiontick.unique()[0]}"
#         if key in close_cache.keys():
...
#         tick_results = list(runThreads(generate_option_tick_new, tickOrderedList, "map", block=True))
#         parsed_opts["opttick"] = tick_results
#         missing_ticks = [
#             x
#             for x in tick_results
#             if x not in close_cache.keys() and x not in oi_cache.keys() and f"{x}_{date}" not in spot_cache.keys()
#         ]
#         data_list = []
...
#         eod_results = list(runThreads(_retrieve_eod_ohlc, OrderedList, "map", block=True))
#         oi_results = list(runThreads(_retrieve_openInterest, OrderedList, "map", block=True))
...
#     def organize_data_for_query(missing_list: list, incomplete_dict: dict, data_request: BulkOptionQueryRequestParameter) -> pd.DataFrame:
...
#     def merge_incomplete_data_in_cache(incomplete_dict: dict, pre_processed_data: pd.DataFrame) -> None:
...
#     def return_closePrice(id: str, date: str) -> float:
#         refresh_cache()
#         global close_cache, spot_cache, oi_cache
#         cache_key = f"{id}"
#         close_data = close_cache[cache_key]
...
#                 pass
#             return None
#         return close_data["Midpoint"][date]

#     def load_chain(date: str, tick: str, right: str = "P", max_close: float = 3.5, order_settings=None, keep: bool = True):
#         order_settings = order_settings or {}
#         chain_saved = Path(os.environ["WORK_DIR"]) / "module_test/raw_code/tickers_data" / "chains"
#         chain_saved.mkdir(parents=True, exist_ok=True)
#         path_to_save = chain_saved / f"{tick}/{date}/"
#         chain_path = path_to_save / "option_data.pkl"
#         if chain_path.exists():
#             return "exists"
#         option_data = retrieve_chain_bulk(tick, target=right, start_date=date, end_date=date)
#         option_data = option_data[(option_data["Midpoint"] <= max_close) & (option_data["Right"] == right)]
#         option_data.to_pickle(chain_path)
#         return f"Saved to {chain_path}"

#     def chain_details(date: str, tick: str, dte: int, rel_strike: float = 0.05, right: str = "P", moneyness_width: float = 0.15) -> pd.DataFrame:
#         try:
#             eqobj = Stock(tick, fetch_close=True)
#             s0 = eqobj.spot()["close"]
#             spot = eqobj.spot(ts=True, ts_start=date, ts_end=date, spot_type="cache")["close"][date].iloc[0]
#         except Exception as e:
#             logger.error(e)
#             return "error"
#         try:
...
#                 end_date = change_to_last_busday(date).strftime("%Y-%m-%d")
#             specific_strike = rel_strike * s0 if right == "C" else rel_strike * (-1) * s0
#             option_details = DataHandler().get_atm_straddle(
#                 symbol=tick,
#                 start_date=end_date.strftime("%Y-%m-%d"),
#                 end_date=end_date.strftime("%Y-%m-%d"),
#                 dte=dte,
#                 specific_strike=specific_strike,
#                 right=right,
#             )
#             option_details["strike_dist"] = option_details["Strike"] - option_details["spot"]
#             option_details["strike_dist"] = option_details["strike_dist"].abs()
#             option_details["is_straddle"] = False
#             strikes = option_details.Strike.unique()
#             straddle_strikes = strikes[np.isclose(np.median(strikes), strikes, rtol=0.001)]
#             option_details.loc[option_details["Strike"].isin(straddle_strikes), "is_straddle"] = True
#             option_details = option_details[(option_details["strike_dist"] < spot * moneyness_width) & (option_details["Strike"] > 0)]
#             if "strike_dist" in option_details.columns:
#                 option_details.drop(columns=["strike_dist"], inplace=True)
#             if "spot" in option_details.columns:
#                 option_details.drop(columns=["spot"], inplace=True)
#             option_details.drop(columns=["C", "P"], inplace=True)
#             option_details["option_id"] = option_details.apply(
#                 lambda x: generate_option_tick_new(
#                     symbol=x["ticker"], exp=x["Expiration"].strftime("%Y-%m-%d"), strike=float(x["Strike"]), right=x["Right"]
#                 ),
#                 axis=1,
#             )
#             return_dataframe = pd.DataFrame()
...
#     def available_close_check(id: str, date: str, threshold: float = 0.7, lookback: float = 30) -> bool:
#         cache_key = f"{id}"
#         if cache_key in DELETED_KEYS:
#             logger.info(f"Close Check: {id} is in DELETED_KEYS, returning False")
#             return False
...
#         for k, v in sample_id.items():
#             if k in new_dict_keys:
#                 transfer_dict[new_dict_keys[k]] = float(sample_id[k]) if k == "strike" else sample_id[k]
#         if cache_key in close_cache:
#             close_data_sample = close_cache[cache_key]
#             close_data_sample = close_data_sample[(~close_data_sample.index.duplicated(keep="first")) & (close_data_sample.index <= date)]
#             close_data_sample = close_data_sample.iloc[-lookback:]
#         else:
#             start = LOOKBACKS[date][lookback]
#             close_data_sample = retrieve_eod_ohlc(**transfer_dict, start_date=start, end_date=date)
#             close_cache[cache_key] = close_data_sample
#         close_mask_series = close_data_sample.Close != 0
#         return close_mask_series.sum() / len(close_mask_series) > threshold

#     def liquidity_check(id: str, date: str, pass_threshold: int | float = 250, lookback: int = 10) -> bool:
