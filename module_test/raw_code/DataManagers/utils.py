...
EMPTY_TIMESEIRES_TABLE = pd.DataFrame(
    {
        "Open": {},
        "High": {},
        "Low": {},
        ...
        if load_name == "s0_close":
            ...
            if market_ts is not None:
                try:
                    ...
                    result = market_ts.get_timeseries(sym=self.symbol, factor="spot", interval=interval)
                    if result and result.spot is not None and not result.spot.empty:
                        logger.info(
                            f"✓ Loaded s0_close for {self.symbol} from MarketTimeseries cache ({len(result.spot)} rows)"
                        )
                        return result.spot
            ...
            if market_ts is not None:
                try:
                    ...
                    result = market_ts.get_timeseries(sym=self.symbol, factor="chain_spot", interval=interval)
                    if result and result.chain_spot is not None and not result.chain_spot.empty:
                        logger.info(
                            f"✓ Loaded s0_chain for {self.symbol} from MarketTimeseries cache ({len(result.chain_spot)} rows)"
                        )
            ...
            r = get_risk_free_rate_helper_v2()["annualized"]
            if intra_flag:
            ...
            intra_flag = kwargs.get("intra_flag", False)
            r = get_risk_free_rate_helper_v2()["name"]
            ...
            if market_ts is not None:
                try:
                    ...
                    result = market_ts.get_timeseries(sym=self.symbol, factor="dividends", interval=interval)
                    if result and result.dividends is not None and not result.dividends.empty:
                        logger.info(
                            f"✓ Loaded dividends for {self.symbol} from MarketTimeseries cache ({len(result.dividends)} rows)"
                        )
                        return result.dividends
                except Exception as e:
                    logger.debug(
                        f"MarketTimeseries cache miss for dividends: {e}, falling back to Stock.div_yield_history()"
                    )
    ...
    def __getitem__(inner, key):  ## Custom getter for EOD Dict. To initialize the data, if not already done
        if key not in inner.parent._intra:
            if key not in ["s0_close", "s0_chain", "r", "y", "r_name"]:
                raise KeyError(
                    f"{key} not in intra data, expected one of: ['s0_close', 's0_chain', 'r', 'y', 'r_name']"
                )
            inner.parent._intra[key] = inner.parent._lazy_load(
                key, ts_timewidth="5", ts_timeframe="minute", intra_flag=True
            )
